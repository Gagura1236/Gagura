...
│
│  ── 主程式（三個版本選一即可）────────────────────────────────
├── sp500_prediction.py          ← 版本 1.0：基本版（終端機執行）
├── sp500_prediction.ipynb       ← 版本 1.0：Jupyter Notebook / VSCode 互動版
├── sp500_prediction_v1.1.py     ← 版本 1.1：新增詳細數值報告圖表
├── sp500_interactive_report.py  ← 互動版：產生互動式 HTML 報告（含懸停說明）
│
│  ── 說明文件 ──────────────────────────────────────────────────
├── README.md                    ← 本說明文件
├── requirements.txt             ← 套件清單（含 macOS 系統依賴說明）
├── implementation_plan.md       ← AI 代理人實作計畫書（v1.1）
│
│  ── 自動產生的輸出檔案 ─────────────────────────────────────────
├── sp500_results.png            ← 1.0：預測對照 + VIX + 誤差（三合一）
├── sp500_feature_importance.png ← 1.0：特徵重要性排名圖
├── model_comparison.csv         ← 1.0：基本評估指標（4項）
│
├── sp500_results_v1.1.png       ← 1.1：預測對照（含數值框 + ±RMSE線）
├── sp500_report_v1.1.png        ← 1.1：詳細報告頁（散點/直方/CDF/滾動RMSE/15項指標表）
├── sp500_feature_importance_v1.1.png ← 1.1：特徵重要性（長條含數值標注）
├── model_comparison_v1.1.csv    ← 1.1：擴充評估指標（15項）
│
└── sp500_interactive_report.html ← 互動版：瀏覽器開啟，含懸停說明與詞彙表
```
...
# 完成後雙擊 sp500_interactive_report.html 開啟
```
